...
class Tracer:
    def __init__(self, data, org, rst_follow=True, nocode=(), saltos=None,
                 reubica=()):
        # reubica: [(ini, fin, ejecuta_en)] -> el trozo ini..fin del binario NO
        # se ejecuta donde esta: alguien lo copia con LDIR a `ejecuta_en` y
        # salta alli. Konami's Game Master lo hace cuatro veces, porque tiene
        # que conmutar la ranura de la pagina 1, que es donde vive el mismo: en
        # cuanto la conmuta, el codigo que estaba ejecutando desaparece de
        # debajo, asi que la maniobra tiene que correr desde RAM.
        #
        # Para el trazador el problema es que las direcciones absolutas de ese
        # trozo apuntan a donde se EJECUTA. Un `jp 0xDA4C` no sale del binario:
        # es un salto al byte 0x4C del propio bloque. Sin esta traduccion el
        # trazado se corta en el primer salto interno y el bloque queda a
        # medias, contado como datos.
        self.reubica = list(reubica)
        ...
        while work:
            ...
            while True:
                ...
                if op == 0xC3:                             # JP nn
                    ...
                    t = self.traduce(pc, self.word(pc + 1))
                    self._add(t, work)
                    if t in self.saltos:
                        k = self.saltos[t]
    ...
    def traduce(self, pc, target):
        """Un destino de salto, visto desde pc, llevado a su sitio del binario.

        Solo traduce si pc esta DENTRO de un bloque reubicado y el destino cae
        en el rango donde ese bloque se ejecuta. Asi un `jp 0xC000` desde fuera
        del bloque sigue siendo lo que es -un salto a RAM que no podemos
        seguir- y solo se traducen los saltos internos del propio bloque.
        """
        for ini, fin, ejec in self.reubica:
            if ini <= pc < fin and ejec <= target < ejec + (fin - ini):
                return ini + (target - ejec)
        return target

    def _add(self, target, work, pc=None):
        if pc is not None:
            target = self.traduce(pc, target)
...
def main():
    ...
    for ln in open(entries_path, encoding="utf-8"):
        ...
        if ln.lower().startswith("!reubica"):
            # !reubica 0x40DE 0x419A 0xDA00  -> el trozo 0x40DE..0x4199 se copia
            # con LDIR a 0xDA00 y se ejecuta ALLI. Sus direcciones absolutas
            # apuntan al destino, no a donde estan guardadas, asi que hay que
            # traducirlas hacia atras para poder seguir el flujo. Ademas de la
            # traduccion, el inicio del bloque se siembra como punto de entrada:
            # a un bloque copiado no se llega siguiendo el flujo.
            _, a, b, e = ln.split()[:4]
            reubica.append((int(a, 0), int(b, 0), int(e, 0)))
            entries.append(int(a, 0))
            continue
    ...
    if reubica:
        print(f"bloques reubicados: {len(reubica)}, "
              f"{sum(b - a for a, b, _ in reubica)} bytes que se ejecutan en RAM")

    t = Tracer(data, org, nocode=nocode, saltos=saltos, reubica=reubica)
    t.trace(entries)
    blocks = t.blocks()
    if t.params:
